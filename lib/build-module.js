...
const alias = {
	...
	querystring: 'querystring-es@^0.2.0',
...
async function buildModule(
	...
	const getPkgJsn = async (fs) => {
		if (!pkgJsn) {
			const pkgJsnStr = await fs.$readFile('/package.json');
			pkgJsn = JSON.parse(pkgJsnStr);
		}
		return pkgJsn;
	};
	...
	const compiler = webpack({
		...
		externals: [
			async function({ context, request }, callback) {
				...
				const isDep = request.match(depNamePtrn);
				...
				const [depName] = isDep;
				console.log({ depName })
				const pkg = await getPkgJsn(compiler.inputFileSystem);
				const version = getVersion(pkg, depName);
				if (!version) {
					console.log(`Couldnt resolve "${depName}" in "${moduleName}"`);
					return callback();
				}

				const resolveTo = request.replace(depNamePtrn, '/' + version).replace(/\/$/, '');

				console.log({ request, resolveTo });
				callback(null, resolveTo);
			},
		],
		optimization: {
			// Too slow
